...
function checkIfWordIsReal(word) {
    ...
    $.ajax({
        ...
        //originally was lasde, but didn't have mellifluous
        url: "https://api.pearson.com/v2/dictionaries/laad3/entries",
        data: {
            headword: word
        },
        success: function(response) {
            ...
            // Otherwise, it is not.
            // var theAnswer = true;
            var thisWordIsAWord = false;
            if (response.results.length === 0){
                thisWordIsAWord = false;
            }
            else {
                thisWordIsAWord = true;
            }


            // TODO 15
            // Update the corresponding wordSubmission in the model
            for (var submissionCounter = 0; submissionCounter < model.wordSubmissions.length; submissionCounter++){
                let currentWord = model.wordSubmissions[submissionCounter];
                if (currentWord.word === word){
                    currentWord.isRealWord = thisWordIsAWord;
                }
            }
...
function render() {
    ...
    var theWordSubmissions = model.wordSubmissions.map(wordSubmissionChip);
    $("#word-submissions").append(theWordSubmissions);
    ...
    var disallowedLetters = disallowedLettersInWord(model.currentAttempt.toLowerCase());
...
function wordSubmissionChip(wordSubmission) {
    ...
    if (wordSubmission.hasOwnProperty("isRealWord")) {
        ...
        if (wordSubmission.isRealWord === true){
            ...
            scoreChip.addClass("realWordScoreChip");
        } else {
            scoreChip = $("<span></span>").text(" X");
            scoreChip.addClass("notRealWordScoreChip");
...
$(document).ready(function() {
    ...
    $("#word-attempt-form").submit(function(evt) {
        ...
        addNewWordSubmission(model.currentAttempt.toLowerCase());
...
function currentScore() {
    ...
    return wordScores.reduce(function(acc, val){
        return acc + val;
    }, 0);
    // return wordScores;
}
